...
#include <limits>   // Memasukkan header file untuk batasan tipe data numerik

using namespace std; // Menggunakan namespace std untuk menghindari penulisan std:: terus menerus
...
int hargaTipeKamar[] = {100000, 200000, 300000}; // Array berisi harga standar, deluxe, suite
...
int kamarTersedia[3][2] = {{1, 1}, {1, 1}, {1, 1}}; // Array untuk menandai ketersediaan kamar
...
// Variabel global
long long totalHarga; // Variabel untuk menyimpan total harga
...
void pilihKamar(Customer* customer) {
  ...
  while (cin.fail()) {
    ...
    cout << "Lama inap harus berupa angka. Masukkan kembali: ";
    cin >> lamaInap;
  }
...
void tampilCustomer() {
  ...
  while (current != NULL) { // Melakukan traversal linked list
